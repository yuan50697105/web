...
<script>
				...
				$("#publishBtn").click(function(){
					...
					if (boxs.length == 0){
						...
						if (confirm("您确定要发布选中的信息？")){
							...
							window.location = "${pageContext.request.contextPath}/product?method=publish&ids=" + ids.get();
						}
					}
				});
			...
			var deleteFn = function(id){
				if (confirm("您确定要删除吗？")){
					window.location = "${pageContext.request.contextPath}/product?method=delete&id=" + id;
				}
			};
			/** 修改信息 */
			var updateFn = function(id){
				window.location = "${pageContext.request.contextPath}/product?method=toUpdate&id=" + id;
			};
			var allPublish=function(){
				if(confirm("您确定要发布所有的信息？")){
					 window.location="${pageContext.request.contextPath}/product?method=allPublish";
				}
			};
		</script>
</head>
<body>
	...
	<div id="wrapper">
		<div id="header" class="clear">
			<div class="search">
				<form action="${pageContext.request.contextPath }/product" method="post" id="searchForm">
					...
					<input type="text" value="${name }" id="title" name="name" class="text">
					<br>
			...
			<table width="98%" class="tab">
				...
				<c:forEach items="${products}" var="p">
					...
					<tr class="field" style="border-bottom: #f3d5d6 1px dashed;">
						<td><input class="checkbox" value="${p.id}" id="box_${p.id }"
							type="checkbox" /></td>
						<td>${p.proType.name }</td>
						<td>${p.id }</td>
						<td>${p.name}</td>
						<td>${p.createTime}</td>
						<td>${p.publishTime }</td>
						...
						<td><a href="javascript:void(0);"
							onclick="deleteFn(${p.id});" style="color: blue;">删除</a></td>
						<td><a href="javascript:void(0);"
							onclick="updateFn(${p.id});" style="color: blue;">修改</a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
		...
		<div align="center">
			...
			<c:if test="${ps != 1 }">
				【<a
						href="${pageContext.request.contextPath }/product?method=listByName&page=1&name=${name}"
						style="text-decoration: underline; color: blue;">首页</a>】
					【<a
						href="${pageContext.request.contextPath }/product?method=listByName&page=${page-1 le 1?1:page-1 }&name=${name}"
						style="text-decoration: underline; color: blue;">上一页</a>】
				<!-- 控制下一页 与 尾页-->
					【<a
						href="${pageContext.request.contextPath }/product?method=listByName&page=${page+1 ge ps?ps:page+1 }&name=${name}"
						style="text-decoration: underline; color: blue;">下一页</a>】 
					【<a
						href="${pageContext.request.contextPath }/product?method=listByName&page=${ps}&name=${name}"
